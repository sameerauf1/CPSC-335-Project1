...
# fuel [1,2,1,0,3]
# mpg 10
def myhamilton(city_distances, fuel_at_gas_station, car_mpg, starting_city):
    number_of_cities = len(city_distances)
    my_dict = {}  # will store city number : fuels left over
    fuel_left_over = 0  # for each round trip, we need to reset left over fuel to zero
    about_to_land_in_city0_from_city_0 = False
    for j in range(starting_city, starting_city + number_of_cities):
        ...
        if fuel_left_over >= 0 and j == starting_city:
            if starting_city == 0 and j == 0:
                continue
            print("we adding now to dict, city: ", myindex,
                  "fuel_left_over: ", fuel_left_over)
...
myhamilton([5, 25, 15, 10, 15], [1, 2, 1, 0, 3], 10, 0)
